...
class CloudBaseGauge extends Component<Props, State> {
    ...
    update(data: any) {
		...
		newState.value=DataUtils.extractInteger(data.cloudbasevalue);
		...
		if(data.cloudbaseunit==='m'){
			...
			newState.maxValue= Math.max(DataUtils.nextHighest(newState.value,1000), this.props.controller.gaugeGlobals.cloudScaleDefMaxm, newState.maxValue);
			...
			newState.maxValue= Math.max(DataUtils.nextHighest(newState.value,2000), this.props.controller.gaugeGlobals.cloudScaleDefMaxft, newState.maxValue);
