...
class BaroGauge extends Component<Props, State> {
	...
	constructor(props: Props) {
		...
		this.params = {
			...
			trendVisible: props.controller.gaugeConfig.showPressTrend,
	...
	async update({press, pressL, pressH, pressTL, pressTH, presstrendval, pressunit}: RtData) {
		...
		if (high === todayHigh && low === todayLow) {
			...
			newState.sections = [];
			newState.areas = [Section(todayLow, todayHigh, this.props.controller.gaugeConfig.minMaxArea)];
		}
		else {
			newState.sections = [
			...
			newState.areas = [
					Section(newState.minValue, low, 'rgba(255,0,0,0.5)'),
					Section(high, newState.maxValue, 'rgba(255,0,0,0.5)'),
					Section(todayLow, todayHigh, this.props.controller.gaugeConfig.minMaxArea)
			];
		}

		if (this.params.trendVisible) {
			// Convert the WD change over 3 hours to an hourly rate
			let trendVal = presstrendval / (this.props.controller.config.weatherProgram === WProgram.WHEATHER_DISPLAY ? 3 : 1);
